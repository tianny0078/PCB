	...
	, m_pfDlg(nullptr)
{
	...
	setWindowTitle(QStringLiteral("Midea ETC PCB 3D Measurement Tool v 0.1"));
	...
	ccConsole::Print("PCB 3D Measurement started!");
}
...
void MainWindow::pcbInitialization() {
	// load pcd file names from config file
	QStringList importFilenames;
	QString filepath = QCoreApplication::applicationDirPath();
	filepath.append("/config.txt");
	//ccConsole::Print(filepath);
	QFile file(filepath);
	...
	while (!in.atEnd()) {
		...
		importFilenames.append(line);
	}
	file.close();

	//// debug dump
	//for (const QString &filename : importFilenames) {
	//	ccConsole::Print(filename);
	//}
	////

	// read PCD files : source and reference
	addToDB(importFilenames, QString(""));
}

void MainWindow::pcbSORfiltering() {
	// find both point clouds for cleaning
	ccHObject* rootObject = m_ccRoot->getRootEntity();
	// get two loaded point cloud
	//ccHObject::Container childObjects;
	//rootObject->filterChildren(childObjects, false, CC_TYPES::HIERARCHY_OBJECT);
	//for (ccHObject* groupObject : childObjects) {


	//ccConsole::Print(QString::number(groupObject->getUniqueID()));
	//ccConsole::Print(QString::number(groupObject->getChild(0)->getUniqueID()));
	//groupObject->setEnabled(false);//this will uncheck the treeview, but not trigger the undisplay (still display)

	//m_ccRoot->selectEntity(cleanCloud, true);
	//}
	// this might be much faster~
	int numChildren = rootObject->getChildrenNumber();
	for (int i = 0; i < numChildren; ++i) {
		ccConsole::Print(QString::number(rootObject->getChild(i)->getUniqueID()));
		ccConsole::Print(QString::number(rootObject->getChild(i)->getChild(0)->getUniqueID()));

		m_ccRoot->selectEntity(rootObject->getChild(i)->getChild(0), true);
	}
	...
	/// test whether the selections are correct
	//ccHObject::Container selectedEntities1 = getSelectedEntities(); //we have to use a local copy: 'selectEntity' will change the set of currently selected entities!

	//for (ccHObject *entity : selectedEntities1)
	//{
	//	ccConsole::Print(QString::number(entity->getUniqueID()));
	//}
	//////////////////////
...
void MainWindow::pcbDistanceComputation() {
	...
	ccGenericPointCloud* refCloud = ccHObjectCaster::ToGenericPointCloud(m_selectedEntities[0]);
	ccGenericPointCloud* compCloud = ccHObjectCaster::ToGenericPointCloud(m_selectedEntities[1]);
...
void MainWindow::pcbFilterByValue() {
	...
	/*
	double minVald = 2.0; // mm
	double maxVald = 15.0; // mm
...
void MainWindow::pcbLabelConnectedComponents() {
	//keep only the point clouds!
	std::vector<ccGenericPointCloud*> clouds;
	{
		for (ccHObject *entity : getSelectedEntities())
	...
	int octreeLevel = 8; // default value
	unsigned minComponentSize = 10; // default value
	...
	for (ccGenericPointCloud *cloud : clouds)
	{
		if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD))
		{
			...
			{
				createComponentsClouds(cloud, components, minComponentSize, randColors, true);
				// manually set the name of first 3 point clouds
				m_selectedEntities[0]->setName(QString("Missing Part!"));
				m_selectedEntities[1]->setName(QString("Wrong Part!"));
				m_selectedEntities[2]->setName(QString("Floating of Part!"));
				m_selectedEntities[0]->toggleShowName();
				m_selectedEntities[1]->toggleShowName();
				m_selectedEntities[2]->toggleShowName();

				ccHObject::Container object;
				for (int i = 0; i < 3; ++i) object.push_back(m_selectedEntities[i]);
				m_ccRoot->unselectAllEntities();
				m_ccRoot->selectEntities(object);
			}
		}
	}
}